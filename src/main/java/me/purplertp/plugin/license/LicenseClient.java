...
public final class LicenseClient {
    ...
    public LicenseClient(PurpleRTP plugin) {
        this.plugin = plugin;
        loadCache();
    }
    ...
    private void performCheck() throws Exception {
        ...
        if (valid) {
            ...
            saveCache();
            plugin.getLogger().info("License OK. " + (customer.isEmpty() ? "" : "Licensed to " + customer + ". ") + LicenseConstants.MADE_BY);
        } else {
    ...
    private void saveCache() {
        try {
            ...
            yaml.set("note", "Cache for offline grace. Deleting this only forces a fresh check. " + LicenseConstants.MADE_BY);
            yaml.save(cacheFile());
        } catch (Exception e) {
